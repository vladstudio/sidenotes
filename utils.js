    ...
    try {
      ...
      if (!isFolder) {
        ...
        if (oldEditor) {
          ...
          await vscode.window.showTextDocument(document, { viewColumn: oldEditor.viewColumn, preview: false });
        }
      }
    } catch (err) {
      showNotification(
  ...
  quickPick.onDidChangeValue(async (value) => {
    ...
    searchTimeout = setTimeout(async () => {
      const searchResults = await searchNotes(
        value,
        sidenoteProvider.sidenotesFolderPath
      );
